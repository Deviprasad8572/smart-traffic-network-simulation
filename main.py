...
# -----------------------------
application = "Smart Traffic Network Simulation"
...
# -----------------------------
def physical_layer(base_delay, load):
    noise = abs(awgn_noise())
    return base_delay * load + noise

# -----------------------------
# STEP 4: Network Layer (Routing)
# -----------------------------
...
def transmit_packet(path):
    ...
    for i in range(len(path)-1):
        ...
        delay = physical_layer(base_delay, traffic_load[link])

        total_delay += delay
        traffic_load[link] += 0.05

        # Packet loss condition (congestion based)
...
print("Smart Traffic Network Simulation Started\n")

for i in range(packets):

    path = choose_best_path()
    delivered, delay = arq(path)
...
# Graph
# -----------------------------
plt.bar(["Delivered", "Lost"], [success, loss])
plt.title("Smart Traffic Network Performance")
plt.xlabel("Status")
plt.ylabel("Packets")

plt.savefig("network_performance.png")  # saves image
plt.show()
